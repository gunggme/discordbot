...
morning_sends = "조례 들어오세요\n링크 : 줌 링크"  # 조례할 때 출력할 문장
afternoon_sends = "종례 시간입니다."  # 종례할 때 출력할 문장


def men(message): 
    firstGradeClass = discord.utils.get(message.guild.roles, name="1-3")
    return firstGradeClass
...
async def time_check(ctx, time):
    ...
    for i in range(0, 6):  # 0~5까지 반복 총 6번 반복 -> 1~6교시 (월~목 7교시 금 6교시)
        ...
        if (time == ttb.All_TIME(6)) and (days(True) != "Wed"):  # 금요일은 6교시이기 때문에 구분
            period = 7
            await sends_delay("time_table")

        if (time == ttb.PM_TIME("default")) and (days(True) != "Wed"):  # 위와 같이 금요일은 6교시라서 월~목 때 종례 알람
            await sends_delay("afternoon_send")  # 종례 시간 일 시 공지

        if (time == ttb.PM_TIME("friday")) and (days(True) == "Wed"):  # 금요일 종례 알람
            await sends_delay("afternoon_send")  # 종례 시간 일 시 공지
